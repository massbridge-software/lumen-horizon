...
{
    function mix($file, $buildDirectory = '../storage/framework/cache')
    {
        static $manifest = [];
        $path = $buildDirectory.'/Mix.json';
        $asset_parts = pathinfo($file);
        $pattern = '/(' . $asset_parts['dirname'] . '\/' . $asset_parts['filename'] . ')/';
